...
*   **fix(undead-kitten):** Fixed positioning issue where the Undead Kitten was completely hidden off-screen because it was placed at the bottom of the document naturally while using `fixed` positioning and `framer-motion` transform. Added `top-0` and `left-0` constraints so it calculates transforms correctly from the top-left of the viewport.
*   **Refactor(UI):** Completely revamped the site design and style to an "Israeli advanced startup vibe". Removed all "metal/hell" themes, updated color palettes to clean tech colors (blue, purple, coral, green), and implemented continuous smooth background animations and idle pulses. Documented changes in `.ai/design-system.md`.
